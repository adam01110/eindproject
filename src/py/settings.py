from lib import get, set_setting
from pyscript import when
from sidebar import read_sidebar_edge_hover_enabled, set_sidebar_edge_hover_enabled
from theme import read_saved_theme, set_theme
...
def set_sidebar_edge_hover_switch_value(is_enabled):
    toggle = get("settings-sidebar-edge-hover-switch")
    if not toggle:
        return
...
async def sync_settings_state():
    ...
    PENDING_THEME = normalize_theme(await read_saved_theme())
...
def set_dialog_open(is_open):
    trigger = get("sidebar-settings-menu-trigger")
    dialog = get("sidebar-settings-dialog")
...
async def on_settings_dialog_close(_event):
    ...
    if dialog and dialog.returnValue == "save" and PENDING_THEME is not None:
        ...
        await set_theme(PENDING_THEME)
        set_sidebar_edge_hover_enabled(enabled)
        await set_setting("sidebar_edge_hover", enabled)
    if trigger:
